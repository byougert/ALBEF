...
def evaluate(model, data_loader, tokenizer, device, config):
    ...
    img2text = json.load(open(config['img2txt_file'], 'r'))
    for img_id, captions_id in metric_logger.log_every(img2text.items(), 5, header):
        img_id = int(img_id)
        order = dataset.img_id2order[img_id]
        image, index = dataset[order]
        image = image.to(device)
        # image: 1 * 3 * 384 * 384
        image = image.unsqueeze(dim=0)
        # image_feat: 1 * 577 * 768
        image_feat = model.visual_encoder(image)
        # image_embed: 1 * 256
        image_embed = model.vision_proj(image_feat[:, 0, :])
        image_embed = F.normalize(image_embed, dim=-1)

        num_text = len(captions_id)
        captions_order = [dataset.text_id2order[cap_id] for cap_id in captions_id]
        text_feats = []
        text_embeds = []
        text_atts = []
        ...
        for i in range(0, num_text, text_bs):
            # list: 256
            text = [texts[order] for order in captions_order[i: min(num_text, i + text_bs)]]
            # text_input.input_ids: 256 * 30, text_input.attention_mask: 256 * 30
            text_input = tokenizer(text, padding='max_length', truncation=True, max_length=30,
        ...
        # val: text_embeds: 1819 * 256
        text_embeds = torch.cat(text_embeds, dim=0)
        # val: text_feats: 1819 * 30 * 256
        text_feats = torch.cat(text_feats, dim=0)
        # val: text_atts: 1819 * 30
        text_atts = torch.cat(text_atts, dim=0)

        # val: sims_matrix: 1 * 1819
        sims_matrix = image_embed @ text_embeds.t()
        # val: score_matrix_i2t: 1819
        score_matrix_i2t = torch.full((num_text,), -100.0).to(device)
        ...
        encoder_output = image_feat.repeat(config['k_test'], 1, 1)
        # encoder_att: 128 * 577
        ...
        score_matrix_i2t[topk_idx] = score
...
def main(args, config):
    ...
    score_result_i2t, score_result_t2i = evaluate(model_without_ddp, test_loader, tokenizer, device, config)
